...
if (posterNameEntered && posterTitleEntered)  // if not entered, then we will use pdata that we built from g parameters.
{
            ...
            for(Iterator j=metaTuples.iterator(); j.hasNext(); ){
                ...
                //set the array elements for the figureObject
                 ...
                 if (pfn.endsWith(".gif") || pfn.endsWith(".jpg") || pfn.endsWith(".png") || pfn.endsWith(".jpeg")){
                   ...
                   //   out.write("<BR>"+fileNumber);
                      ArrayList foL = new ArrayList();
                        if (figureObj.size() > 0) {
                           for (int i=0; i<figureObj.size(); i++) {
                          foL = (ArrayList)figureObj.get(i);

                             String optionLfn=(String)foL.get(0);
                             String optionPfn=(String)foL.get(1);
                               
                       out.write(" lfn="+optionLfn+" pfn="+ optionPfn + "<BR>");
                            }
                        }
