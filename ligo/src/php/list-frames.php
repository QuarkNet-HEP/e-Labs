 ...
 *
 *   -d <dir>       root directory in which data files can be found. If
 *                  missing, some heuristics will be attempted.
 *
 *
...
$MAX_FILES = 1024;              // some sanity check on home many files to produce
                                // since user can specify crazy ranges
...
while( $GPS_prefix > 748 ) { 
    ...
    // No actual check for existence. We only want this script to
    ...
    //////////////
        ...
        echo "[" . $Nfiles . "] " . $dest_gwf . "\n";
        $Nfiles++;
        if ($Nfiles > $MAX_FILES) {
        	echo "Max number of files exceeded.\n";
        	exit(4);
        }
        //}
    }// next frame (earlier)
...
//}
if( $Nfiles==0 ) {
	echo "NO FRAME FILES WERE FOUND!\n";
	exit(5);
}
