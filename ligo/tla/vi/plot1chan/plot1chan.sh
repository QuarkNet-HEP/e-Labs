...
#
#  app {
#        plot1chan.sh taskid GPSstart GPSend chName tAxisPref 
#      }  
#
# Environment:
...
# @(#) $Id: plot1chan.sh,v 1.7 2008/08/22 21:00:45 myers Exp $
#######################################################################
...
do      case "$1" in
        --dataDir|-D)                   
	     ...
	     shift ;;
       --version|-V)  
             VER=`echo "$Revision: 1.7 $ " | awk '{print $2}' `
             DATE=`echo "$Date: 2008/08/22 21:00:45 $ " | awk ' {$1=""; $NF=""; print'} `
...
ROOT_CMD="\$TLA_VI_DIR/plot1chan/plot1chan.C(\"$TaskID\",\"$chName\",$GPSstart"
ROOT_CMD="$ROOT_CMD,$GPSend,\"$ttype\",\"$tAxisPref\")"
