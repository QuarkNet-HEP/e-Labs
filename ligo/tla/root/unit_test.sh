#!/bin/sh 
#
# Run the test ROOT script unit_test.C 
# using the script run_dmt_test.sh 
#
# Swift Signature:
#
#  app {
#        unit_test.sh taskid GPSstart GPSend chName 
#      }  
#
# Environment:
...
# @(#) $Id: unit_test.sh,v 1.1 2009/02/11 19:59:48 myers Exp $
#######################################################################
...
usage(){
    cat <<EOT
Usage:
      `basename $0` TaskID GPSstart GPSend chName --dataDir /path/to/data 
EOT
   exit 0		     
}
...
do      case "$1" in
       ...
       --version|-V)  
             ...
             DATE=`echo "$Date: 2009/02/11 19:59:48 $ " | awk ' {$1=""; $NF=""; print'} `
...
echo "Channel: $chName "
if [ -z "$chName" ]; then
  echo "$MYNAME requires a channel name.  "
  exit 33
fi
...
ROOT_CMD="\$TLA_ROOT_DIR/${ROOTscript}(\"$TaskID\",\"$chName\",$GPSstart"
ROOT_CMD="$ROOT_CMD,$GPSend,\"$ttype\")"
