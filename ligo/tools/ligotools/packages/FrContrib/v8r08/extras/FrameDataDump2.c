...
int main(int argc, char *argv[])
{
	...
	int Iflag = 0, Oflag = 0, Nflag = 0, Tflag = 0, zeroFlag = 0, displayFlag = 0;
	...
	for(i = 1; i < argc; i++)
	{
		...
		switch(*(argv[i] + 1))
		{
			...
			case 'D':
				...
				if((i >= argc) || (*argv[i] == '-') )
				{
					debugPath = malloc(9*sizeof(char));
...
/*This tells the user what inputs were incorrect, if any*/
	if(Iflag != 1 || (Oflag != 1 && displayFlag != 1))
	{
					...
					if (strstr(adc->name, ".mean") || strstr(adc->name, ".rms")) {
						...
						if(fileNum == 1 && frameNum == 1) {
							...
							if(adc == NULL && proc == NULL && sim == NULL) {
								...
								if(adc != NULL) {
									//adc->sampleRate seems to lie on occasion
									//srate = adc->sampleRate;
									srate = 1.0 / adc->data->dx[0];
								}
						...
						if(fabs(timeStamp-prevStamp)>RESOLUTION) {
							...
							//fprintf(stderr, "\n********************************************************************************\n");
							if (!seqWarn) {
								fprintf(stderr, "Warning: frames not sequential (%.3f second gap)\n", t);
								seqWarn = 1;
							}
						...
						if(displayFlag == 0) {
							fprintf(outInfo, "%d samples (%.3f s) successfully written\n", nWritten, (double)nWritten/srate);
							pointCount -= nWritten;
						}
