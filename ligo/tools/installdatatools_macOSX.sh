...
############
# EPeronja-05/10/2013: Needed to make a ton of changes to get this to install the LIGO tools in 
#					   in my MAC OSX Mountain Lion.
############
...
############
# Peter Shawhan suggested changing the code below
############
#step "Installing Fr"
...
step "Downloading Fr"
wget -a $LOG -O Fr_v8r08_SOURCE.tar.gz http://www.ldas-sw.ligo.caltech.edu/ligotools/Fr/v8r08/Fr_v8r08_SOURCE.tar.gz >>$LOG 2>&1

step "Installing Fr"
# "y" to "do you want to make this the active version?"
echo "y" | ligotools_install Fr_v8r08_SOURCE.tar.gz >>$LOG 2>&1


step "Downloading FrContrib"
wget -a $LOG -O FrContrib_v8r08_SOURCE.tar.gz http://www.ldas-sw.ligo.caltech.edu/ligotools/FrContrib/v8r08/FrContrib_v8r08_SOURCE.tar.gz >>$LOG 2>&1

step "Installing FRContrib"
# "y" to "do you want to make this the active version?"
echo "y" | ligotools_install FrContrib_v8r08_SOURCE.tar.gz >>$LOG 2>&1
############
...
#$DIR/ligotools/packages/FrContrib/v8r08/extras/ >>$LOG 2>&1

cp /Users/edit/source/i2u2/ligo/tools/ligotools/packages/FrContrib/v8r08/extras/* $DIR/ligotools/packages/FrContrib/v8r08/extras/ >>$LOG 2>&1

############
# Changed the following path because there is no Makefile in the active folder
############
#cat <<END | patch $DIR/ligotools/packages/FrContrib/active/Makefile >>$LOG 2>&1
cat <<END | patch $DIR/ligotools/packages/FrContrib/v8r08/Makefile >>$LOG 2>&1
7c7
...
pushd $DIR/ligotools/packages/FrContrib/v8r08 >>$LOG 2>&1
...
popd >>$LOG 2>&1
############
# Changed the following path because I did not download FrameDataDump2.c to the active folder
############
#ln -s $DIR/ligotools/packages/FrContrib/active/bin/FrameDataDump2 $DIR/ligotools/bin/FrameDataDump2 >>$LOG 2>&1
ln -s $DIR/ligotools/packages/FrContrib/v8r08/bin/FrameDataDump2 $DIR/ligotools/bin/FrameDataDump2 >>$LOG 2>&1
############
# Added more links to make it work locally
############
ln -s $DIR/ligotools/packages/Fr/v8r08/bin/FrCheck $DIR/ligotools/bin/FrCheck >>$LOG 2>&1
ln -s $DIR/ligotools/packages/Fr/v8r08/bin/FrCopy $DIR/ligotools/bin/FrCopy >>$LOG 2>&1
ln -s $DIR/ligotools/packages/Fr/v8r08/bin/FrDump $DIR/ligotools/bin/FrDump >>$LOG 2>&1
ln -s $DIR/ligotools/packages/Fr/v8r08/bin/make_myfrgetvect $DIR/ligotools/bin/make_myfrgetvect >>$LOG 2>&1
