...
sub new () {
  ...
  $self->{_cmdlHashRef} = $cmdl_hash if defined $cmdl_hash || undef;
  bless $self, $class;
...
sub getCmdlHashRef {
  my ($self) = @_;
  return $self->{_cmdlHashRef};
}

sub getCmdlParam {
  my ($self, $param) = @_;
  return $self->{_cmdlHashRef}->{$param} if defined $self->{_cmdlHashRef}->{$param} || undef;
}
