...
<%
        ...
        if (!(rsId==null)) id =  Integer.parseInt(rsId);  // id of research_group
        boolean includeDetectors = false;  // set true if the research group is registered to use "cosmic" e-Lab
        ...
        if (submit != null && submit.equals("Update Group Information")) {
            if(group != null && project != null && ay != null && role != null && survey != null && passwd1 != null && passwd2 !=null){
                ...
                if(project.equals("cosmic") && detectorString != null && !detectorString.equals("")){
                    i=0;
                    SQLstatement = 
        ...
        if (submit != null && submit.equals("Show Group Info")) {
            ...
            if (group != null && !group.equals("Choose Group")) {
                ...
                // Get project information for this group to see if you should include the detector fields needed for cosmic.
                // The assumption here is that you should show it if they are assigned to the cosmic e-Lab even if you are in the cms e-Lab now.
                // This is looking forward to a time when updateGroups.jsp might not be associated with a particular e-Lab.
                // It could also have been implemented so that you only show it if the code is running in the cosmic e-Lab.
                query = 
                ...
                rs = s.executeQuery(query);
                while (rs.next()) {
                    String legalProject=rs.getString("name");
                    if (legalProject.equals("cosmic"))  includeDetectors=true;
                  }
               if (includeDetectors) {
...
<%
                                ...
                                //any characters the filesystem cannot handle must be taken care of
                                    ...
                                    if(group == null){
                                        ...
                                        out.write(group);
                                        out.write("<input type=\"hidden\" name=\"group\" value=\"" + group +"\"><input type=\"hidden\" name=\"id\" value=\"" + id +"\">\n");
                                    }
                                    out.write("</td></tr>");

                                    out.write("<tr><td align=\"right\">e-Lab:</td><td>");
                                   //Note: This really does not work now because we are not really changing anything related to the project unless we wanted to change the detectors for a group using code from a different e-Lab.
                                   // and comment out the code
                                   if (project==null || project.equals("") ) project =elabName;   // set it to the default for the elab we are currently in if it is not passed
                                   // rs = s.executeQuery("SELECT name FROM project");
                                   
                                     rs = s.executeQuery("SELECT name FROM project WHERE id IN (" + "SELECT project_id FROM research_group_project " + "WHERE research_group_id = " + id + ");");
                                    out.write("<select name=\"project\">");
                                   while(rs.next()){
                                       String selected = "";
                                       if (rs.getString(1).equals(project)) selected="selected";
                                       out.write("<option value=\"" + rs.getString(1) + "\" " + selected + ">" + rs.getString(1) + "</option>\n");
                                   }
                                    // out.write("<option value=\"" + project + "\">" + project + "</option>\n");  // This version was allowing only one choice.
                                    out.write("</select>\n");
                                    out.write("</td></tr>");

                                    out.write("<tr><td align=\"right\">Academic Year:</td><td>");
                                    // Changed to default to the current academic year.  LQ: July 24, 2006
...
%>
                                    <select name="ay">
                                        ...
                                        <option value="AY2007">2008-2009</option>
                                    </select>
<%
                                    }
                                    else{
...
<%
                                    ...
                                    out.write("</td></tr>");
                                    // conditional code for cosmic only
                                    if (includeDetectors) {
										...
										out.write("</td></tr>");
                                    } // end of test for cosmic project
...
%>
                            ...
                            <tr>
                                ...
                                <table cellpadding=3>
<%
                               // Changed to only show students for which there is not survey answers.  We do not want student records to disappear whose indices are in the answer table.
                               String studentsQuery="";
                               try {
                                    studentsQuery = 
                                        "SELECT id, name FROM student WHERE id not in (select distinct student_id from answer) and id in (" +
                                    ...
                                    out.write("<font color=\"red\">Problem retrieving your students using query</font><BR>"+ studentsQuery );
                                    out.flush();
                                }
%>
                                </table>
    		...
    		<P>To assign groups to more e-Labs, go to <A HREF="updateGroupProjects.jsp">Assign groups to new e-Lab.</></p>
