...
<%-- 12Dec2017 Updated to protect from XSS attacks - JG --%>
...
"&plots=" + entry.getTupleValue("_plots") + "&analysis=" + entry.getTupleValue("analysis") + "&cuts=" + cuts;
...
<c:choose>
		<c:when test="${rasterized}">
				<img src="${url}"/>
				<br/>
				<%-- Use fn:escapeXml(param.blah) to prevent XSS attacks --%>
				<a href="../data/view-metadata.jsp?filename=${fn:escapeXml(param.filename)}&menu=${fn:escapeXml(param.menu)}">Show details (metadata)</a>
		</c:when>
		<c:otherwise>		
				<a href="../data/view-metadata.jsp?filename=${fn:escapeXml(param.filename)}&menu=${fn:escapeXml(param.menu)}">Show details (metadata)</a>&nbsp;|&nbsp;
				<a href="../data/plot.jsp?${fn:escapeXml(params)}&combine=on">Edit this plot</a>
