...
if(get.equals("meta")){
    ...
    pfn = getPFN(filename);
	String metaPlotPath="FIXME";
    
    meta = getMeta(filename);
    String metaTeacher,metaGroup,metaState,metaCity,metaSchool,metaYear;
    metaYear=metaSchool=metaCity=metaState=metaGroup=metaTeacher="";	
    
    if(meta != null){
		//out.println("meta is "+meta.toString());  	
        for(Iterator i=meta.iterator(); i.hasNext(); ){
            Tuple t = (Tuple)i.next();
            String metaKey=(String)t.getKey();
            if(t.getValue()==null){
            	out.println("Missing metadata entry for "+metaKey);	
				continue;
            }
            if(t.getValue() instanceof String){
	            String metaValue=(String)t.getValue();
				if("teacher".equalsIgnoreCase(metaKey))
					metaTeacher=metaValue;	
            	if("group".equalsIgnoreCase(metaKey))
            		metaGroup=metaValue;
    			if("state".equalsIgnoreCase(metaKey))
    				metaState=metaValue;	
    	        if("city".equalsIgnoreCase(metaKey))
    	           	metaCity=metaValue;
    			if("school".equalsIgnoreCase(metaKey))
   					metaSchool=metaValue;	
   	            if("year".equalsIgnoreCase(metaKey))
    	           	metaYear=metaValue;
            }else{
            	out.println(t.getValue()+" not a String");
            }    
        }
        metaPlotPath=metaYear+"/"+metaState+"/"+metaCity+"/"+metaSchool+"/"+metaTeacher+"/"+metaGroup+"/cms/plots";
        
    }
    else{
		out.println("ERROR: META IS MISSING");
    }
    
	metaPlotPath=metaPlotPath.replaceAll(" ","_");
    
    pfn="/elab/cms/output/"+metaPlotPath+"/"+filename;
	//out.println("pfn reconstructed "+pfn);
    //pfn="/elab/cms/output/AY2006/IN/notre_dame/ND_QN_Center/Beth_Marchant/cmsguest/cms/plots/"+filename;
