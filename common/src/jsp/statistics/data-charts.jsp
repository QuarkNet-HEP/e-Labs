...
<c:set var="type" value="${param.type != null ? param.type : 'split'}"/>
...
<table border="0" id="active" class="lefty small">
	...
	<c:forEach var="e" items="${stats.mostActiveDataUsers}" varStatus="li">
...
<table border="0" id="monthly" class="lefty small">
	...
	<c:forEach var="e" items="${stats.monthlyDataCounts}" varStatus="li">
		<tr style="background-color: ${li.count % 2 == 0 ? '#ffffff' : '#e0e0e0' }">
			<td>${e.key}</td>
			<td>${e.count}</td>
			<td style="vertical-align: middle; text-align: left;">
