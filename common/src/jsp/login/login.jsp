...
if (!maxLoginsReached) {
		...
		if (user != null) {
				...
				/* 'prevPage' tells us where the user logged in from so we can send
				 * them back there after successful login.
				 * We must check that it's our domain to prevent its GET parameter
				 * from being used as a cross-site scripting attack.
				 */

				String[] domains = {"../", "i2u2.org", "elabs.i2u2.org", "i2u2-prod.crc.nd.edu"};

				String prevPage = request.getParameter("prevPage");

				// Parse `prevPage` to make sure it comes from an acceptable domain
				String ppg = prevPage.replaceFirst("https://", "");
        ppg = prevPage.replaceFirst("http://", "");
        ppg = prevPage.replaceFirst("http://", "");
        ppg = prevPage.replaceFirst("www.", "");

        boolean validDomain = false;
        //for (int i = 0; i < domains.length; i++) {
        //    if ( ppg.startsWith(domains[i]) ) {
        //        validDomain = true;
        //}

				int i = 0;
        while (!validDomain && i < domains.length ) {
            if ( ppg.startsWith(domains[i]) ) {
                validDomain = true;
            }
            i++;
