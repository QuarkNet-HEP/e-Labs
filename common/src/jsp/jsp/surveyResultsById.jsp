...
if (role == null || !role.equals("admin")) { //Short circuit operator.  Do not change order.%>
<tr><td>
To see the test results of students, you need to have a admin login. If you have a teacher login, log in with it.  If you need help, contact <A HREF="mailto:quarknet@fnal.gov">quarknet@fnal.gov</A>.
</td></tr>
</table>
<%
return;
...
try {
    ...
    if (teacherUA == null || teacherID == -1)
     ...
     String teacher_id= request.getParameter("teacher_id");  // now get the teacher id that has been passed

    rs = s.executeQuery("select name from teacher where id = '" + teacher_id + "'");
    if (rs.next()) {
...
<TR><TD>
	...
	<B>Results for  <%=prePost%>test:   <%=teacher_name%>'s students</B>
	</TD></TR></table>
