...
if (!(pass==null) && pass.equals("switchingelabs")) {
    rs = s.executeQuery("SELECT password FROM research_group WHERE " +
                        "name='" + userName +  "';");
    if(rs.next() == false){
        incorrectLogin = true;
      }
       ...
       // First we want to get the project id for the current project name.  LQ-7/21/06
        ...
        if( (rs2.next() == false) && !(role.equals("teacher"))  && !(role.equals("admin")) ){
           ...
           //  project = rs2.getString(1);  This is left over from Paul's query.  We already know the project name.  LQ-7/21/06
                ...
                if (prevPage.endsWith("home.jsp")) {
            ...
            } else if (firstTime || survey.equals("t")) {
            
                   int countQuestions=0;
                   rs=s.executeQuery("select count(*) from question where question.project_id="+ projectId +" AND question.test_name='pretest';");
                   if (rs.next()) countQuestions=rs.getInt("count");
                   ...
                   if(rs.next()) {
                      ...
                      if ((countStudents > 0) && (countQuestions > 0)) {
                            ...
                            if (firstTime)
                            {
                            // reset firsttime so you don't come here again
                              try{
                                 SQLstatement="UPDATE research_group SET first_time='f' WHERE id = \'" + groupID + "\';";
                                 i = s.executeUpdate(SQLstatement);
                                    } catch (SQLException se){
                                    warn(out, "There was some error updating info in the research_group table.\n<br>Please contact the database admin with this information:\n<br>SQLstatement: " + SQLstatement);
                                  return;
                                  }
                              response.sendRedirect(
                              response.encodeRedirectURL(
...
Please log in to enter the <%=project%> e-Lab</B></font>
           </TD>
       <TR>
    </TABLE>
<%
        ...
        <TABLE align = center BORDER=0 WIDTH=500 CELLPADDING=4 VALIGN=TOP>
            <TR>
                ...
                <TD align=center>
                                ...
                                <FORM method="post">
                                    <TABLE border= 0 cellpadding=2 cellspacing=10>
                                        <TR>
                                            ...
                                            {
                                            %>
                                            <TD><input size="16" type="text" name="user" tabindex="1" value=""></TD>
                                            <%
                                            }
                                            else
                                            {
                                             %>
                                            <TD><input size="16" type="text" name="user" tabindex="1" value="<%=userName%>"></TD>
                                             <%
                                            }
                                            %>                                            
                                       </TR>
                                        <TR>
                                            ...
                                            <TD><INPUT size="16" type="password" name="pass" tabindex="2"><INPUT type="hidden" name="project" value="<%=project%>"></TD>
                                        </TR>
                                        <TR>
                    ...
                    <TR>
                        <td align="center" colspan=3><font align="center" face="ariel"><a href=login.jsp?prevPage=<%=prevPage%>&user=<%=guestName%>&pass=guest&project=<%=project%>>Login as guest</a></font></td>
                    </TR>
                </td>
            </tr>
        </table>
