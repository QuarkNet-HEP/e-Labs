...
if($#ARGV < 4){
	die "usage: Plot.pl -in [input-file1] -in [input-file2 etc.] -extra [extrafunctions file] -param [parameter filename] -svg [.png filename] -type [plot type] optional for all(-title [title label] -ylabel [ylabel] -xlabel [xlabel] -caption [caption] -lowx [x lowerbound] -highx [x upperbound] -lowy [y lowerbound] -highy [y upperbound]) optional for 3D graphs(-zlabel [zlabel] -lowz [z lowerbound] -highz [z upperbound])\n\tplot type: 0 - histogram, 1 - line, 2 - 3d, 3 - lifetime-histogram w/ best-fit line, 4 - Scatter, 5 - TEST. 6 - 3d w/o lines, 7 - histogram with color and multiple files\n";
}

my %h = ();
#my @infile = ();
my $result = GetOptions(\%h, 'in=s@', 'extra=s', 'param=s', 'svg=s', 'type=i', 'title=s', 'ylabel=s', 'xlabel=s', 'zlabel=s', 'caption=s', 'lowx=s', 'highx=s', 'lowy=s', 'highy=s', 'lowz=s', 'highz=s');
...
sub ticLevels(){ # this is used for the flux study to find out where the tic marks should be and set them
    ...
    $monthLow = substr($dateLow[0], 1); ## we need to take them out or it messes up timegm
    $dayLow = $dateLow[1];
    $yearLow = $dateLow[2];
    $hourHigh = $timeHigh[0];
    $minuteHigh = $timeHigh[1];
    $secondHigh = substr($timeHigh[2], 0, 2);
    $monthHigh = substr($dateHigh[0], 1);
    $dayHigh = $dateHigh[1];
    $yearHigh = $dateHigh[2];
...
#call gnuplot which makes the .png file
if(-x "/usr/local/bin/gnuplot"){ 
    `/usr/local/bin/gnuplot $outfile_param`;
}
else {
    `/opt/gnuplot/gnuplot-4.0.0/bin/gnuplot $outfile_param`;
}
