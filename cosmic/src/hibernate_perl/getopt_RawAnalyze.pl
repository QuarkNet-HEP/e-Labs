...
if($#ARGV < 3){
	die "usage: $0 -in [input-file] -out [output-file] -gate {[event gatewidth]}\n";
}

my %h = ();
my $result = GetOptions(\%h, 'in=s', 'out=s', 'gate=i');
...
$infile = $h{'in'};
$outfile = $h{'out'};
$gatewidth = defined($h{'gate'}) ? $h{'gate'} : 240;	#default gatewidth of 240ns if not specified
