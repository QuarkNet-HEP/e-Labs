...
use Getopt::Long;
if($#ARGV < 2){
	die "usage: ThresholdTimes.pl -in [input-file1] -in [input-file2 etc.] -out [output-file1] -out [output-file2 etc.] -id [board-ID1] -id [board-ID2 etc.] -cpld [cpld-freq1] -cpld [cpld-freq2 etc.]  You have ".($#ARGV+1)." arguments and they are \"@ARGV\"\n";
}
my %h = ();
my $result = GetOptions(\%h, 'in=s@', 'out=s@', 'id=i@', 'cpld=s@');
...
sub print_data {
    ...
    if($nanodifference > 100 or $nanodifference < 0){  #physically impossible to have a time over threshold > 100ns or < 0ns
