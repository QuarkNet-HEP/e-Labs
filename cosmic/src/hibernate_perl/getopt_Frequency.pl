...
if($#ARGV < 4){
	die "usage: Frequency.pl -in [input-file1] -in [input-file2 etc.] -out [output-file1] -out [output-file2 etc.] -col [column number of file (start at 1)] -binType [1/0 1:binwidth 0:number of bins] -binValue [binwidth or number of bins, depending on what you chose]\n";
}
my %h = ();
my $result = GetOptions(\%h, 'in=s@', 'out=s@', 'col=i', 'binType=i', 'binValue=s');

@infile = @{$h{'in'}};
@outfile = @{$h{'out'}};
$col = $h{'col'} - 1;
$binType = $h{'binType'};		#set to 1 if using binwidth, 0 if using numberOfBins
$binValue = eval $h{'binValue'};
