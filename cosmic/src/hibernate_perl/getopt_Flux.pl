...
if($#ARGV < 3){
	die "usage: Flux.pl -in [input-file] -out [output-file] -bin [bin width (in seconds)] -geo [base geometry directory path]\n";
}

my %h = ();
my $result = GetOptions(\%h, 'in=s', 'out=s', 'bin=i', 'geo=s');
...
$binwidth=(eval $h{'bin'})/86400;	#convert seconds to partial days
$fluxFound=0;
$geo_dir = $h{'geo'};
...
sub freq_analyze {
    if($#data != 0){ #frequency module doesn't work with one data point
        ...
        while($keyIterator<= $#keysWanted){
            my $current_flux = ($freq{($keysWanted[$keyIterator])}/(eval $h{'bin'})/$areaArray[$keyIterator]);
