...
if($#ARGV < 4){
	die "usage: EventChoice.pl -in [input-file] -out [output-file] -num [event number] -zero [zero-zero-zero point ID] -geo [geometry directory path]\n";
}

my %h = ();
my $result = GetOptions(\%h, 'in=s', 'out=s', 'num=i', 'zero=i', 'geo=s');
...
$ofile = $h{'out'};
$event_num = $h{'num'};
$zerozeroID = $h{'zero'};	#the ID of the detector that we're choosing to be 0-0-0 on the graph that we make
$geo_dir = $h{'geo'};	#base directory for geometry (cosmic.datadir in the Quarknet JSP files)
