...
if($#ARGV < 11){ 
    die "usage: lifetime.pl -in [input-file] -out [output-file] -gate [gatewidth in seconds] -check [0 or 1 for check energy] -coin [required coincidence Level 0 to n] -geo [geometry directory]\n";
}

my %h = ();
my $result = GetOptions(\%h, 'in=s', 'out=s', 'gate=s', 'check=i', 'coin=i', 'geo=s');
...
die "Error: Lifetime gatewidth ($h{'gate'}) must be positive\n" if($h{'gate'}<=0);
warn "Error: Lifetime gatewidth ($h{'gate'}) being more than 1 second will yield lots of noise\n" if($h{'gate'}>1);
$offset = $h{'gate'}/86400; 
die "Error: CheckEnergy ($h{'check'}) value needs to be zero or one\n" if($h{'check'}!=0 && $h{'check'}!=1);
$checkEnergy = $h{'check'};	#the program checks the energy of the second signal, making sure it is smaller than the first signal
die "Error: coincidence level has to be bigger or equal to zero\n" if($h{'coin'}<0);
$minimumCoincidenceLevel = $h{'coin'};
