...
public class ElabVDS {
    ...
    private static void dbConnect() throws ElabException{
        if(dbschema!=null) // DB is already open
            return;
        try{
     ...
     * @param  meta        key/type/value triples each separated by a space
     ...
     * provenance, rawanalyze, thumbnail.
     *
                ...
                if(key.equals("provenance")){
                    ...
                    if(f.canWrite()){
                        ...
                        throw new ElabException("Could not delete file: " + pfn + 
                                "...Either it doesn't exist or the system doesn't have " + 
