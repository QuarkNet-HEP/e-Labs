...
<div id="<%= request.getParameter("chartName") %>" >
	<table id="<%= request.getParameter("chartName")%>Table" >
		<tr>
			...
			Y axis scale: </td>
			<td style="background-color: lightGray">Max Y: <input type="text" id="<%= request.getParameter("chartName")%>MaxY" /><input type="button" value="Set" id="<%= request.getParameter("chartName")%>YButton" onclick='javascript:redrawPlotY(<%= request.getParameter("chartName")%>MaxY.value, "<%= request.getParameter("chartName")%>");' /></td>
		</tr>
	</table>
</div>		