...
<div id="<%= request.getParameter("chartName") %>" >
	<table id="<%= request.getParameter("chartName")%>Table" >
		<tr>
			...
			<td style="background-color: lightGray"><input type="button" value="Reset" id="<%= request.getParameter("chartName")%>ResetYButton" onclick='javascript:resetPlotY("<%= request.getParameter("chartName")%>","<%= request.getParameter("chartName")%>MinY", "<%= request.getParameter("chartName")%>MaxY" );' /></td>
		</tr>
	</table>
</div>		