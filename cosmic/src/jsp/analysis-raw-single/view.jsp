...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body id="view-rawdata" class="data">
...
<table border="0" id="main">
	<tr>
		<td id="center">
<%
	String[] parameters = java.net.URLDecoder.decode(request.getQueryString(), "UTF-8").split("&");
	String filename, highlight, line1;
	if (parameters == null) {
	    throw new ElabJspException("Please choose a file to view");
	} else {
		filename = parameters[0].replace("filename=","");
		highlight = parameters[1].replace("highlight=","");
		line1 = parameters[2].replace("line=","");
	}
...
%> 
		...
		<c:if test="${e.tupleMap.detectorid != null}">
			<a href="../geometry/view.jsp?filename=${filename}">Show Geometry</a> |
		</c:if>
		<c:if test="${e.tupleMap.blessfile != null}">
			<a href="../analysis-blessing/compare1.jsp?file=${filename}">Show Blessing Charts</a> |
		</c:if>
		<a href="../data/download?filename=${filename}&elab=${elab.name}&type=split">Download</a>
		<br/>
		<br/>
		...
		<%
			if(moreLines){
			    ...
			    %>
			    	<a href="?filename=${filename}&highlight=${highlight}&line=${count}">Next 100 lines...</a>
			    <%
			}
		%>
	<%
