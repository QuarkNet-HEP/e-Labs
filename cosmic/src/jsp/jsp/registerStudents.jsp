...
<TR><TD>
	...
	<B>e-Lab Student Registration</B>
	</TD></TR></table>
...
int projectId=0;
rs = s.executeQuery("select id from project where name = '" + project + "'");
    if (rs.next()) {
        ...
        try {
            ...
            for (int formNum = 0; formNum < 10; formNum++) {
                ...
                if (!researchTracker.containsKey(resName) && !isOldGroup) {
                    ...
                    "values((select id from research_group where name = '" + resName + "'), "+ projectId + ")"); 
                     int exitCode = 0;
                ...
                if (survey != null && survey.equals("yes"))
                    s.executeUpdate(
                        ...
                        "(select id from student where name = '" + studentName + "'), "+ projectId + " )"); 
                studentsAdded++;
            }
        } catch (Exception e) {
            valid = false;  
...
<td>
    ...
    action="https://<%=System.getProperty("host")+System.getProperty("sslport")%>/elab/<%=project%>/registerStudents.jsp"> 
    <tr>
