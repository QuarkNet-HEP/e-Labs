										...
										<tr>
												<td id="center">
														<c:if test="${geometryNotFound}">
																<e:error message="No geometry found for detector ${fn:escapeXml(param.detectorID)} on date ${fn:escapeXml(param.jd)}"/>
														</c:if>
														
														<c:if test="${param.filename != null}">
																<center>
																		<a href="../data/view.jsp?filename=${fn:escapeXml(param.filename)}">Show Data</a> |
																		<a href="../data/view-metadata.jsp?filename=${fn:escapeXml(param.filename)}">Show Metadata</a>
																</center>
														</c:if>
														
														<div id="viewer">
																<div class="title">
																		<a href="javascript:glossary('geometry')">Geometry</a> for ${param.filename == null ? fn:escapeXml(param.id) : fn:escapeXml(param.filename)}
																		<c:if test="${param.filename == null}">
