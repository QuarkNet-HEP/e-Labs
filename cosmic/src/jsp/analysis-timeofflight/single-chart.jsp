...
<div class="left">
	...
	<% if (!user.isGuest()) { %>		
			...
			<input type="button" name="save" onclick='return validatePlotName2("chartName<%= request.getParameter("chartIndex")%>"); return saveToFChart(<%= request.getParameter("chartIndex")%>, "chartName<%= request.getParameter("chartIndex")%>", "msg<%= request.getParameter("chartIndex")%>", <%= request.getParameter("runId") %>); ' value="Save Chart"></input>     
