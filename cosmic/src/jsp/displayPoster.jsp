...
<%
   ...
   if (dfile==null)
   {
   warn(out, "No poster name has been chosen.");
   }
   { // ok to display it. because poster name is passed.
     ...
     if(pfn==null) 
         throw new RuntimeException("Could not resolve LFN "+lfn+" to PFN.");
   ...
   HashMap tags = new HashMap();
      ...
      String tfile = "template_"+templateType+".htmt"; //paper version of the template
      String template = "";
      try {
        ...
        warn(out, "EXCEPTION! reading template" + " " + e.getMessage() );
      }
      ...
      if(pf2.exists()) {
        ...
        try {
         // FileReader fr = new FileReader(posterDir+dfile+".data");
          ...
          warn(out," EXCEPTION! reading poster file" + " " + e.getMessage() );
        }
