	...
	<c:when test="${param.rawData != null}">
		...
		<table border="0" id="main">
			<tr>
				<td id="center">
					<p>
						<a href="../jsp/dpstutorial.jsp">Understand the graph</a>
					</p>
				    ...
				    <c:if test="${!(empty performance.invalidParameters) && param.submit != null}">
				    	<h2>Invalid keys:</h2>
				        <ul class="errors">
				            <c:forEach var="f" items="${performance.invalidParameters}">
				                <li><c:out value="${f}"/></li>
				            </c:forEach>
				        </ul>
				    </c:if>
