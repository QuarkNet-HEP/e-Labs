...
<%@ page import="org.griphyn.common.catalog.replica.SimpleFile" %>
...
//System.setProperty("vds.home", "/home/nepywoda/vds-1.2.14/");
if (pf.canRead()) {
...
if (user == null ) {
    ...
    response.sendRedirect(
            ...
            System.getProperty("sslport") + "/elab/cosmic/login.jsp?prevPage="+prevPage);
    return;
}
...
/**
  ...
  */
public static String getPFN(JspWriter out, String lfn){
    String pfn = null;
    try {
        SimpleFile rc = new SimpleFile();
        rc.connect(ChimeraProperties.instance().getRCLocation());
        pfn = rc.lookup(lfn, "local");
    }
    catch (Exception e) {
        ;
        //out.write("<CENTER><FONT color= red>Error while looking up: " + lfn + "</FONT><BR>" + e + "<BR></CENTER>");
    } 
    return pfn;
}
%>
...
/**
  ...
  *                     <code>false</code> otherwise.
  */
public static boolean setMeta(JspWriter out, String filename, java.util.List meta){
    // Connect the database.
    DatabaseSchema dbschema = null;
    try{
        String schemaName = ChimeraProperties.instance().getVDCSchemaName();
...
public static boolean addRC(JspWriter out, String lfn, String pfn){
    ...
    try {
        ...
        //out.write("<CENTER><FONT color= red>Error updating RC entry..." + e + "</FONT><BR><BR></CENTER>");
        ;
    } 
        ...
        if (! (dbschema instanceof Annotation)) {
            ...
            for(Iterator i=metaTuples.iterator(); i.hasNext(); ){
                ...
                try{
                    ...
                    if (annotationschema != null)
                        annotationschema.close();
                    throw new ElabException("Error while deleting the annotation key: " + t.getKey() + " - " + e.getMessage());
                }
            }
        } 
    } catch(Exception e){
...
public static boolean deleteLFN(JspWriter out, String lfn){
    ...
    try{
        ...
        //out.println("<CENTER><FONT color= red>Error connecting to the database ...</FONT><BR><BR>" + e + "<BR></CENTER>");
        ;
    } finally {
        try {
            if (dbschema != null)
    ...
    try{
    //out.println("allmod: "+allModified+"|<BR>");
    }catch(Exception e){
        ;}
    ...
    if(fileDeleted){
        ...
        try{
            ...
            //out.println("<CENTER><FONT color= red>Error deleting from rc.data...</FONT><BR><BR>" + e + "<BR></CENTER>");
            ;
        }
    }
...
public static String getDBStatusMessage() throws ElabException{
    ...
    try{
        ...
        if (! (dbschema instanceof Annotation)) {
            ...
            try {
                annotationschema = (AnnotationSchema)dbschema;
                java.sql.ResultSet rs = annotationschema.backdoor("SELECT COUNT(*) FROM anno_string WHERE value = 'split'");
                if (rs.next())
                    status = "Searching " + rs.getString(1) + " data files";
                rs = annotationschema.backdoor(
                    "select count(distinct value) from anno_string where id in (select id from anno_lfn where mkey = 'school')");
                if (rs.next())
                    status += " from " + rs.getString(1) + " schools";
                rs = annotationschema.backdoor(
                    "select count(distinct value) from anno_string where id in (select id from anno_lfn where mkey = 'state')");
                if (rs.next())
            ...
            throw new ElabException("DB threw exception: " + e.getMessage() + "\n");
    } finally {
        try {
            if (dbschema != null)
                dbschema.close();
...
public static void svg2png(String svgFilename, String pngFilename, String thumbPngFilename, String pngHeight, String thumbPngHeight) throws ElabException{
    ...
    try {
        ...
        input = new TranscoderInput(svgFile);
        ostream = new FileOutputStream(pngFilename);
        output = new TranscoderOutput(ostream);
        trans.transcode(input, output);
        ostream.flush();
        ostream.close();
    } catch (Exception e){
        throw new ElabException("Error while transcoding svg to output png filename " + pngFilename + " with height " + pngHeight + ":" + e);
    }
    ...
    try{
        ...
        throw new ElabException("Error while transcoding svg to output thumbnail png filename " + thumbPngFilename + " with height " + thumbPngHeight + ":" + e);
    }
}
%>
