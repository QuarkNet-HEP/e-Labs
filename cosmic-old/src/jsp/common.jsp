...
if (user == null && (!loginTarget.endsWith("/login.jsp")) ) {
    ...
    response.sendRedirect(
            ...
            System.getProperty("port") + "/elab/cosmic/login.jsp?prevPage="+loginTarget);
    return;
}
...
public static void warn(JspWriter out, String error){
    try{
        out.write("<span class=\"warn\">" + error + "</span>");
    }
    catch(Exception e){
        ;
    }
...
public static boolean addRC(String lfn, String pfn) throws ElabException{
    ...
    try {
		int c;
		//[m] at least in the SimpleFile case, we need to make
		//sure that no two concurrent accesses to the rc file
		//will overlap
		synchronized(ReplicaCatalog.class) {
	        ReplicaCatalog rc = ReplicaFactory.loadInstance();
	        c = rc.insert(lfn, pfn, pool);
			rc.close();
		}
...
public static boolean deleteLFN(JspWriter out, String lfn){
    ...
    if(fileDeleted){
        ...
        try{
//TODO benc -- this doesn't actually seem to delete anything...
// not sure what's going on here. it was previously code that 
// just connected to the simple file replica and then did nothing
        }
        catch (Exception e) {
