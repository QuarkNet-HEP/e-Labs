...
// Always pass keyword, not id so we can pick off the description
   if (keyword==null) {keyword="general";} //default to general keyword if none is included.
     ...
     query="select id,description from keyword where project_id="+project_id+" and keyword=\'"+keyword+"\';";
     rs = s.executeQuery(query);
     while (rs.next()){
...
<P>
    ...
    // look for any previous log entries for this keyword
     ...
     query=query+ "where log.id=comment.log_id and log.keyword_id=keyword.id and keyword.id=" + keyword_id + " and log.research_group_id=" + research_group_id + " and log.project_id="+project_id+ " and keyword.project_id=log.project_id"; 
