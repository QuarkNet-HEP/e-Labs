...
try {
    ...
    rs = s.executeQuery("select name from research_group where teacher_id= '" + teacherID + "' Order by name;");
    
    while (rs.next())
    {
    allNames[nameId]=rs.getString("name");
    nameId++;
    
    }
    nameId=0;
    rs = s.executeQuery("select student.name as name,count(*) from answer,question,student where answer.question_id=question.id and answer.answer=question.answer and student.id=answer.student_id and student.id in (select DISTINCT student_id from research_group_student,research_group,student where student.id=research_group_student.student_id and research_group_student.research_group_id=research_group.id and research_group.id in (select id from research_group where teacher_id= '" + teacherID + "')) group  by name order by name UNION select student.name as name,count(*) from answer,question,student where answer.question_id=question.id and answer.answer! =question.answer and student.id=answer.student_id and student.id in (select DISTINCT student_id from research_group_student,research_group,student where student.id=research_group_student.student_id and research_group_student.research_group_id=research_group.id and research_group.id in (select id from research_group where teacher_id= '" + teacherID + "')) group  by name order by name;");
	...
	while (allNames[nameId]!=rs.getString("name"))
	{
	student.add(0,allNames[nameId]);
	student.add(1,"0");
