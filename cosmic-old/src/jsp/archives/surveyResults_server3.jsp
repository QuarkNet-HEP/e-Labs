...
try {
   ...
   if (debug)
	correctTable += "</table><br>";
   	...
   	rs = s.executeQuery("select student.name as name,question.id as question_number, CASE WHEN answer.answer=question.answer THEN '<font color=green>*</font>' ELSE '<a href=surveyQuestion.jsp?responseNo=' || answer.answer || '&questionNo=' || question.question_no || '><font color=red>' || answer.answer || '</font></a>' END as response from answer,question,student where answer.question_id=question.id and student.id=answer.student_id and student.id in (select DISTINCT student_id from research_group_student,research_group,student where student.id=research_group_student.student_id and research_group_student.research_group_id=research_group.id and research_group.id in (select id from research_group where teacher_id='" + teacherID + "')) order by name;");
