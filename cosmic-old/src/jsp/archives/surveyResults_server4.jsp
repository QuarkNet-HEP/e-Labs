...
try {
   ...
   if (debug)	
    ...
    ListIterator k = studentList.listIterator();
         ...
         rs = s.executeQuery("select count(*) from answer,question,student where answer.question_id=question.id and answer.answer=question.answer and student.id=answer.student_id and student.name='"+student_name+"';");
        if (rs.next()) {
   ...
   if (debug)
    ...
    String htmlQuery="select student.name as name,question.id as question_number, CASE WHEN answer.answer=question.answer THEN '<a href=surveyQuestion.jsp?responseNo=' || answer.answer || '&questionNo=' || question.question_no || '><font color=green>' || answer.answer || '</font></a>' ELSE '<a href=surveyQuestion.jsp?responseNo=' || answer.answer || '&questionNo=' || question.question_no || '><font color=red>' || answer.answer || '</font></a>' END as response from answer,question,student where answer.question_id=question.id and student.id=answer.student_id and student.id in (select DISTINCT student_id from research_group_student,research_group,student where student.id=research_group_student.student_id and research_group_student.research_group_id=research_group.id and research_group.id in (select id from research_group where teacher_id='" + teacherID + "')) order by name;";
    if (color.equals("no")) htmlQuery="select student.name as name,question.id as question_number, CASE WHEN answer.answer=question.answer THEN '<font color=green>*</font>' ELSE '<a href=surveyQuestion.jsp?responseNo=' || answer.answer || '&questionNo=' || question.question_no || '><font color=red>' || answer.answer || '</font></a>' END as response from answer,question,student where answer.question_id=question.id and student.id=answer.student_id and student.id in (select DISTINCT student_id from research_group_student,research_group,student where student.id=research_group_student.student_id and research_group_student.research_group_id=research_group.id and research_group.id in (select id from research_group where teacher_id='" + teacherID + "')) order by name;";
...
<TR><TD>
	...
	<B>Presurvey Results:   <%=login%>'s students</B><FONT>
	</TD></TR>
...
<BR>
<%
if (color.equals("yes")){%><A HREF="surveyResults.jsp?color=no">Display for Black and White Printout</A><%}else{%><A HREF="surveyResults.jsp?color=yes">Display for Color Printout</A><%}
%>
</td></tr>
</table>
</center>
